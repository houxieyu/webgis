...
var basezbunits = ['人', '亩'];
var zbnames = ['lwbj_cun.AREA_CODE'];
var zbunits = ['人', '亩'];
...
//     serverip = result.ip;
// });
var densityURL = "http://" + serverip + "/arcgis/rest/services/lwdensity/MapServer";
//叠加区县边界
...
function CreateGEOTool(zbnamestr, zbs) {
    require([
        ...
        function (
            ...
            //空间查询图层
            var featureLayer;
            var url = "http://" + serverip + "/arcgis/rest/services/lwbj/MapServer/9";
            ...
            dojo.connect(drawToolbar, "onDrawEnd", function (geometry) {
                ...
                featureLayer.selectFeatures(selectQuery, FeatureLayer.SELECTION_NEW, function (features) {
                    var total = new Array();
                    for (izb = 0; izb < basezbnames.length; izb++)
                        total[izb] = 0;
                    for (i = 0; i < features.length; i++) {
                        //查询bigdata
                        var ft = bigdata.find(x => x['区划码'] == features[i].attributes['lwbj_cun.AREA_CODE']);
                        if(!ft)break;
                        for (izb = 0; izb < basezbnames.length; izb++) {
                            total[izb] +=Number(ft[basezbnames[izb]]=='NULL'?0:ft[basezbnames[izb]]);
                        }
                    }
                    ...
                    for (izb = 0; izb < basezbnames.length; izb++) {
                        ...
                        if (zbunits[izb])
                            units = zbunits[izb];
                        zbstr += basezbnames[izb] + ":<font color='blue'>" + Math.round(total[izb] * 100) / 100 +
                            units + "</font><br/>";
                    }
                    r += zbstr + '</b>';
                    $("#querytext").html(r);
